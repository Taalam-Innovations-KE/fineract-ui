...
export const loanRescheduleRequestSchema = z.object({
	...
	rescheduleReasonId: z.number().int().positive("Reschedule reason is required"),
	...
	if (data.adjustedDueDate && data.rescheduleFromDate) {
		...
		if (!Number.isNaN(adjusted.getTime()) && !Number.isNaN(fromDate.getTime())) {
			if (adjusted.getTime() < fromDate.getTime()) {
				ctx.addIssue({
					code: z.ZodIssueCode.custom,
					path: ["adjustedDueDate"],
					message: "Adjusted due date must be on or after reschedule from date.",
				});
			}
		}
	}
